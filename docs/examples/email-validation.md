...
```python
# enigma: good enough, real validation is sending the mail
...
```python
# enigma: stdlib covers this
from email.utils import parseaddr
"@" in parseaddr(email)[1]
```

**27 lines -> 1 line.** And the honest answer: let the confirmation email reject
it. That is what confirmation emails are for. Note enigma still keeps the real
trust boundary - the confirmation step - it just refuses to fake it with a regex.
