...
First question (rung 1 of the ladder): **do you actually need a cache?**
...
**120 lines -> 0-3 lines.** The fastest cache is the one you did not have to
debug. If a shortcut is taken with a known ceiling, the `enigma:` comment names
it (e.g. `# enigma: in-process lru_cache, move to Redis when we run >1 worker`).
