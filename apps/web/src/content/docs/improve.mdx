...
> Looking for the CLI subcommands (`enigma install`, `enigma autoskills`, ...)? Those are in [Commands](/enigma/docs/commands/). `/improve` runs *inside the agent*, not the `enigma` binary - though `enigma improve --help` prints the same reference.
