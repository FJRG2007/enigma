...
const { items, notify, dismiss } = useNotifications();
...
<Installation item="notifications" pkg="@enigmax/primitives" entry="@enigmax/primitives/react">
    <div slot="manual">

The queue renders nothing. Point it at whatever toaster you already have, or copy the
Tailwind recipe `enigma add notifications --copy` writes.
