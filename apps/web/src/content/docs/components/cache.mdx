...
// Five components, one loader call: the cache dedupes the in-flight request.
const { data, loading, refresh } = useCached("projects", loadProjects, { ttl: 8000 });
```
    </div>
    <div slot="next">
```tsx
...
<Installation item="cache" pkg="@enigmax/utils" entry="@enigmax/utils/react">
    <div slot="manual">
