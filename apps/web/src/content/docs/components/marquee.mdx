...
export const tailwindRecipe = recipe("@enigmax/primitives", "marquee.tailwind.tsx");
export const cssRecipe = recipe("@enigmax/primitives", "marquee.css.tsx");
export const cssSheet = recipe("@enigmax/primitives", "marquee.css");
...
<Preview title="hoverScale: 0" hint="Rest a MOUSE on it and it stops; a tap on touch does not, which is the whole point of the pointerType gate below.">
    <MarqueeDemo speed={70} hoverScale={0} />
    <div slot="code">
```js
// hoverScale multiplies the cruise speed while a MOUSE rests on the row.
// 0 stops it, 0.15 slows it to a crawl, 1 ignores hover entirely.
createMarquee(lane, track, { speed: 70, hoverScale: 0 });
```
...
<StyleTabs title="Marquee.tsx">
    <div slot="tailwind">
        <pre><code class="language-tsx">{tailwindRecipe}</code></pre>
    </div>
    <div slot="css">
        <pre><code class="language-tsx">{cssRecipe}</code></pre>
        <pre><code class="language-css">{cssSheet}</code></pre>
    </div>
</StyleTabs>
...
```jsx
<Marquee items={logos} speed={70} hoverScale={0.15}>
    {(logo) => (
        <a href={logo.href}>
