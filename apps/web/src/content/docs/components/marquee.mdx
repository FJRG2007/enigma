...
<Marquee items={tools} speed={70}>
    {(tool) => <span className="pill">{tool.name}</span>}
</Marquee>
```
    </div>
    <div slot="next">
...
<LogoMarquee
    logos={[
        { kind: "img", src: "/logos/claude.svg", alt: "Claude Code" },
        { kind: "img", src: "/logos/openai.svg", alt: "OpenAI Codex" },
        { kind: "node", node: <span>BLOOMBERG</span> }
    ]}
    speed={55}
    tone="brand"
/>
...
<Frameworks>
    <div slot="react">
```tsx
// The mask lives in your CSS - a colour is a look, and the primitive ships none.
<LogoMarquee logos={logos} speed={55} className="tinted" />
```
    </div>
    <div slot="next">
```tsx
...
<MarqueePlayground client:only="react" />
...
<Frameworks>
    <div slot="react">
```tsx
<Marquee items={tools} speed={70} hover="pause">
    {(tool) => <span className="pill">{tool.name}</span>}
</Marquee>
```
    </div>
...
<Frameworks>
    <div slot="react">
```tsx
<Marquee items={top} speed={110}>{(item) => <span className="pill">{item}</span>}</Marquee>
<Marquee items={bottom} speed={110} reverse>{(item) => <span className="pill">{item}</span>}</Marquee>
```
    </div>
    <div slot="next">
```tsx
