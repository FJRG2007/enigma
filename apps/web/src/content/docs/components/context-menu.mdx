...
A submenu that lists files, branches, tags or devices is not short. Past eight rows a level
gets its own filter - `searchable` on the row decides it outright - matching the label, the
description, the group and any `keywords`, fuzzy if you hand over Fuse's constructor:
