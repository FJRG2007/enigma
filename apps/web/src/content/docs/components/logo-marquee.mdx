...
Some of them are not images, though - Bloomberg, NASA and Harvard are set in type - and some
need a name. The three forms mix in one list:
...
| | Default | |
| --- | --- | --- |
| `logos` | | A URL string, `{ kind: "img", src, alt?, height? }`, or `{ kind: "node", node, key? }` |
