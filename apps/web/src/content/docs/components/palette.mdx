...
{/* The site's own palette reads this and stands down here, so the demo owns Ctrl+K on this
    page and the two never fight over the key. It is static markup rather than something the
    demo renders, because the site's palette hydrates first and would not see it yet. */}
<div data-palette-demo hidden></div>
