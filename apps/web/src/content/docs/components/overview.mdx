...
Beside them, **`@enigmax/icons`** holds the glyphs: a filled duotone set with one module per icon, so a bundle carries only what it renders.
...
- [Context menu]({docHref("components/context-menu")}) - the right-click menu: submenus, shortcuts, a filter when a level is long, rows fetched on demand.
- [Image]({docHref("components/image")}) - a picture that opens into a viewer: press to enlarge, wheel to zoom, with the gallery arrows, previews, download menu and discard action off until asked for.
- [Video]({docHref("components/video")}) - a player shaped after Plyr, with the element as the source of truth and the platform's own shortcuts.
- [Selection list]({docHref("components/selection")}) - the file-manager selection model, with every shortcut rebindable or removable.
- [Icons]({docHref("components/icons")}) - the filled duotone set, searchable by name and category, one module per glyph.
