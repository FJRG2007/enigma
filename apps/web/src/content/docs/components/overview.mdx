...
<Cmd cmd="enigma add" desc="List the catalogue for this project's framework." />
<Cmd cmd="enigma add marquee" desc="Add it as a dependency." />
<Cmd cmd="enigma add marquee --copy" desc="Copy the source into the project, yours to edit." />
<Cmd cmd="enigma add --all" desc="Everything in the catalogue." />
...
```sh
npm install @enigmax/primitives @enigmax/utils
```

## Which framework

The core of every primitive is framework-agnostic TypeScript with no dependencies, so **vanilla JS and Astro work today** through the main entry. React has a thin adapter at `@enigmax/primitives/react`. Vue, Svelte and TanStack adapters are not written yet; they will reuse the same core rather than reimplement it.
