...
- **`diff`** runs only in the **turn-end sweep**, against the lines the current change **added**. There is no legacy backlog by construction, so the rule can only fire on code the agent just wrote.
...
- **opencode** - an auto-loaded plugin that runs after each write/edit. It has no turn-end hook that can answer the model, so diff-stage rules do not run there.
