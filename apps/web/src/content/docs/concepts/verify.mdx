...
No argument checks the current change and runs your verification command when there is new work to check - the same check the turn-end hook runs, on demand. `--all` sweeps every tracked file. It returns a non-zero exit code when something is unfinished, and also when it could not check at all (outside a git repository), so it works as a pre-merge or CI check.
</Cmd>
