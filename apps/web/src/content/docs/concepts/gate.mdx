...
- **Fix, and ask about the rest** (`assisted`, the default) - the agent settles what the review found and comes back only when the answer is not already yes: the finding contradicts what you asked for, undoes a deliberate decision, or takes a very large change. It is never asked whether to repair a defect.
