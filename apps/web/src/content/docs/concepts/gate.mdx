...
- **`ask-user`** - the review could not settle it mechanically. The pipeline stops, and it reaches you only if fixing it would go against what you asked for; otherwise the agent authorizes the fix.
...
- **Fix, and ask about the rest** (`assisted`, the default) - the agent settles what the review found and comes back only when the answer is not already yes: the finding contradicts what you asked for, undoes a deliberate decision, changes agreed behavior, or takes a very large change. It is never asked whether to repair a defect.
- **Fix without asking** (`auto`) - the agent settles everything and never checks back.
- **Always ask** (`ask`) - nothing is fixed until you say so; every finding reaches you first.
...
A finding below your threshold is still found and reported on the run, it just does not stop the pipeline for a decision - only a finding at or above it does. Raise the bar when a diff keeps parking on one more low-priority finding per round instead of converging:

<Cmd name="enigma config gate-severity error" ex={["enigma config gate-severity error -l", "enigma config gate-severity", "enigma config gate-severity warning -g"]}>
Lowest severity that halts a run: `error`, `warning` (the default), or `info`. `-l` scopes it to this project, `-g` to every project. Every pause you answer with `fix` costs the pipeline a full re-review of the diff, so this is the knob for a run that will not stop finding nice-to-haves.
</Cmd>

## Automatic driving

You do not have to ask your agent to run the gate or initialize repos by hand. The deployed agent memory carries an always-on instruction: once the agent has committed a code task, it **drives the gate itself** on whatever branch that work is on - running `enigma gate init` once if the repo is not set up yet - then handles findings the way `fix_policy` says (by default: applies every fix it can, and comes back only for an `ask-user` finding whose repair would go against what you asked for), and leaves the PR ready for you to review and merge. It merges on its own only when you told it to - otherwise it reports the run as waiting on you, not on it.
...
- **`checks-passed`** - validated and CI green; the PR is open for you to review and merge. The run now waits on you, not on the pipeline - `enigma gate axi status` and the dashboard both show it as `needs you: merge the PR` instead of `running`. Merge it yourself with `enigma gate axi merge`, or ask your agent to.
