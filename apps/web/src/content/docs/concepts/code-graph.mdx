...
Manage: `index [path]` builds the graph (defaults to the current directory), `projects` lists indexed projects, `arch [project]` prints the architecture overview, `search <name>` finds symbols by name, `on`/`off` toggle the tools.
