...
Shared flags: `--in <path>` narrows to a subtree, `--limit N` caps results, `--project <name>` picks a graph, `--json` prints the raw result, `--no-refresh` skips the pre-query re-index.
</Cmd>
