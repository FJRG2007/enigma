...
`enigma ssh <alias|name>` connects (both are unique connect keys; `--name` sets the second); `add`/`edit` save credentials - a stored password is auto-filled with **no extra tools** (enigma acts as OpenSSH's SSH_ASKPASS); `tunnel <alias> <name|spec...>` opens port forwards only (`9090:db:5432`, `D:1080`, or a saved tunnel's name); `forward add <alias> SPEC [name]` saves a (nameable) tunnel; `info` shows a connection; `remove` deletes one.
</Cmd>
