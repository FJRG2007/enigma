...
Codebase memory / code graph (structural code intelligence) powered by codebase-memory-mcp, registered in your agents over MCP. Off by default. See <a href={docHref("concepts/code-graph")}>Codebase memory</a> for detail.

<Cmd name="enigma codegraph <status|on|off|projects|arch>" ex={["enigma config code-graph on", "enigma codegraph status", "enigma codegraph projects", "enigma codegraph arch"]}>
`status` reports whether it's registered and available; `on`/`off` register or remove the MCP server across your agents; `projects` lists indexed projects; `arch [project]` prints a project's architecture. Other actions pass through to the tool.
</Cmd>
