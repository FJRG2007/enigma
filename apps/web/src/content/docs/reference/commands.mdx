...
<Cmd name="enigma pack <list|install|remove|update|setup|run>" ex={["enigma pack list", "enigma pack install helio", "enigma helio", "enigma pack setup helio"]}>
`install <id>` fetches a pack; `enigma <id>` (e.g. `enigma helio`) launches its isolated agent, seeded with your active login; `setup <id>` registers the pack's MCP servers (needs Python); `update`/`remove` keep or delete it.
</Cmd>
