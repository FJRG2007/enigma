...
<Cmd name="enigma ssh <alias> | <list|add|edit|remove|info|tunnel|forward>" ex={["enigma ssh add server1 --host 203.0.113.10 --user root -i ~/.ssh/id_ed25519", "enigma ssh server1", "enigma ssh tunnel db 9090:5432", "enigma ssh forward add db 9090:5432 pg", "enigma ssh tunnel db pg"]}>
`enigma ssh <alias>` connects; `add`/`edit` save credentials (passwords stored encrypted, auto-filled via sshpass/plink); `tunnel <alias> <name|spec...>` opens port forwards only (`9090:db:5432`, `D:1080`, or a saved tunnel's name); `forward add <alias> SPEC [name]` saves a (nameable) tunnel; `info` shows a connection (password redacted); `remove` deletes one.
</Cmd>
