...
- A concrete case the user names is an EXAMPLE OF A CLASS, not the whole job - "this label overflows", "this endpoint is unvalidated", "make this button an icon", "this panel has no empty state". Fixing only the instance guarantees the same report comes back for each of its siblings. So: state the general rule the example implies, sweep the repo with deterministic commands (grep/AST/type search) for every other place that rule applies, fix them all in this same change, and report the rule plus the sites touched. Then encode the rule where it holds without you - a file-local mechanical signature becomes a lint/guardrail/CI rule, a domain convention goes in the owning policy skill, a truly universal one goes in this memory - never the same rule in two of those. Stay on the single instance only when the user scoped it there ("only here", "just this one") or when generalizing would need a destructive action or a business decision, and then name what you left unfixed.
