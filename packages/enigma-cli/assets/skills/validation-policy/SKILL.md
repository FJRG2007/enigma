...
- **Empty is not invalid.** A field the user has just cleared, or has not filled in yet, is INCOMPLETE - it is not wrong. Rendering "Email is not valid" the moment the last character is deleted scolds the user for a state they are deliberately passing through, and it is the most common way a real-time form turns hostile. Two states, two treatments:
  - **Empty (after trimming) and required** -> no error text at all. Say it structurally instead: a `*` on the label with `aria-required="true"`, one legend near the form explaining the marker ("* Required"), and the submit button disabled until every required field holds a value. The reason stays within reach, per frontend-policy's rule that a disabled action must say why.
  ...
  - **Empty and optional** -> valid, silent, and submittable. An `.optional()` field must accept `""` as "not provided" rather than reporting a format error on nothing.
- The transition matters as much as the states: when a field that was showing a format error is cleared, that error must DISAPPEAR, not be replaced by "Required". The user knows the field is empty - they emptied it. Errors that only make sense after an attempt to submit (and only where the submit button is not disabled) may name the empty required fields then, and focus the first one.
- This is a schema-shape decision, not a rendering hack: keep the required check and the format check separate (`z.string().min(1, "Required").pipe(z.email())`, not one regex that fails on `""`), so the UI can tell "nothing here yet" apart from "this value is wrong" instead of inferring it from the message text.
