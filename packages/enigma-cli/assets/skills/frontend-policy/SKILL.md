...
## Two Panes Means Two Scrolls (Independent Scroll Regions)

A view with a list on one side and a record on the other has TWO scrollable regions, and they are independent: scrolling the open email moves the email, and the list of emails stays exactly where it was. The same holds for a file tree beside an editor, a thread beside a conversation list, a table beside a details drawer, a chat beside its transcript.

The defect is scroll CHAINING, and it is the most-missed part of any multi-pane layout: the wheel over the reading pane scrolls it to its end and then keeps going into whatever is behind it, so the list jumps, or the whole page moves, or both panes travel together as if they were one document. It looks fine until content is long enough to reach an end, which is why it survives review.

Three things make a pane scroll on its own. All three are required, and the second is the one usually missing:

- **The page does not scroll; the shell is the viewport.** The outer container is `height: 100dvh` (`h-dvh`) with `overflow: hidden`, and the panes inside it own every scrollbar on screen. If the page itself can scroll, nothing below matters - the chain has somewhere to go.
- **Each pane declares `min-height: 0`** (`min-w-0` for a horizontal one). A flex or grid child defaults to `min-height: auto`, which is its CONTENT's height, so the pane grows to fit instead of scrolling and pushes the scrollbar out to the page. This is the same trap as the `min-width: 0` one that makes text refuse to truncate, and it produces no error, no warning and no visual clue except that the wrong thing scrolls.
- **Each pane sets `overscroll-behavior: contain`** (`overscroll-contain`). That is what actually stops the chain: reaching the end of the email does not hand the rest of the gesture to the list behind it. Use `none` only where the browser's own overscroll effect must go too (pull-to-refresh, rubber-banding).

Then the state that goes with them:

- **Each pane's scroll position is its own.** Opening a different record resets the reading pane to the top (`ref.scrollTo({ top: 0 })` on the id changing, not on every render) and must not touch the list's position. Coming back to the list finds it where it was left - restoring a list to the top after a back navigation is the same defect seen from the other side.
- **A pane that scrolls must be reachable from the keyboard.** A `div` with `overflow-y: auto` is not focusable, so Page Down scrolls the page instead of the pane the reader is looking at. Give it `tabindex="0"` and an accessible name (`role="region"` plus `aria-label`), or put a focusable element inside it.
- **On a phone the two panes are two screens**, not two scrollers side by side: the list navigates to the record and back. Two independent scroll boxes stacked in one viewport is the shape nobody can use.

Verify it mechanically rather than by eye, because eyeballing is what let it through: scroll the inner pane to its end, keep scrolling, and assert the OUTER container's `scrollTop` is unchanged. `pane.scrollTop === pane.scrollHeight - pane.clientHeight && outer.scrollTop === before` is the whole check.
