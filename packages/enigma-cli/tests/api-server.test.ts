...
test("agents.parseClaudeLine maps init, assistant text_final, partial text deltas, result and errors", () => {
    ...
    // A full (non-partial) assistant message is the whole answer -> text_final, not text.
    expect(agents.parseClaudeLine('{"type":"assistant","message":{"content":[{"type":"text","text":"Hello "},{"type":"text","text":"world"}]}}'))
        .toEqual({ kind: "text_final", text: "Hello world" });
    // Partial-message deltas (--include-partial-messages): a text_delta streams as an incremental
    // `text` event; a thinking_delta and every non-delta stream event carry no answer content.
    expect(agents.parseClaudeLine('{"type":"stream_event","event":{"type":"content_block_delta","index":0,"delta":{"type":"text_delta","text":"Hel"}}}'))
