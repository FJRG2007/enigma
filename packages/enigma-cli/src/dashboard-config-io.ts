...
export function exportBundle(): ConfigBundle {
    ...
    for (const tool of acct.TOOL_NAMES) {
        tools[tool] = {
            ...
            accounts: acct.listAccounts(tool).filter((a) => a.name !== acct.DEFAULT_NAME).map((a) => a.name),
        };
    }
    const items: Record<string, Record<string, string>> = {};
    for (const p of acct.listProfiles()) items[p.name] = p.accounts;
    return {
        kind: BUNDLE_KIND,
        ...
        accounts: { tools, profiles: { active: acct.getActiveProfile()?.name ?? null, items } },
    };
}
    ...
    if (acc && typeof acc === "object") {
        ...
        for (const tool of acct.TOOL_NAMES) {
            ...
            for (const nameRaw of spec.accounts) {
                ...
                if (acct.accountExists(tool, name)) { skipped.push(`${tool}/${name} (exists)`); continue; }
                try {
                    const a = acct.addAccount(tool, name);
            ...
            if (spec.active && acct.accountExists(tool, spec.active)) { try { acct.setActive(tool, spec.active); } catch { /* */ } }
        }
        ...
        if (profs && profs.items && typeof profs.items === "object") {
            for (const [pname, mapping] of Object.entries(profs.items)) {
                try {
                    if (!acct.listProfiles().some((p) => p.name === pname)) acct.addProfile(pname);
                    for (const [tool, account] of Object.entries(mapping || {})) {
                        if (acct.TOOL_NAMES.includes(tool) && acct.accountExists(tool, String(account))) {
                            try { acct.setProfileAccount(pname, tool, String(account)); } catch { /* skip bad mapping */ }
                        }
                    }
                    applied.push(`profile ${pname}`);
                } catch (err) { skipped.push(`profile ${pname} (${(err as Error).message})`); }
            }
            if (profs.active) { try { acct.setActiveProfile(profs.active); } catch { /* */ } }
        }
    }
