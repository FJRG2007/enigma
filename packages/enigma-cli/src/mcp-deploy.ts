...
export function winWrapInvocation(tool: string, base: string, baseArgs: string[]): { command: string; args: string[] } {
    ...
    return { command: base, args: baseArgs };
}
...
function invocation(tool: string): { command: string; args: string[] } {
    return winWrapInvocation(tool, resolveBin("enigma") ?? "enigma", ["mcp"]);
}

/** Config file that holds `tool`'s MCP servers at `scope`, or null if none applies. */
export function mcpPath(tool: string, scope: Scope): string | null {
    switch (tool) {
        case "claude":
...
export function mcpAccountPath(tool: string, dir: string): string | null {
    switch (tool) {
...
/**
 * Merge or delete the `serverName` entry under `parentKey` in a JSON config file,
 ...
 */
export function applyJsonEntry(file: string, parentKey: string, serverName: string, entry: unknown | null): boolean {
    ...
    if (entry === null) delete parent[serverName];
    else parent[serverName] = entry;
...
/** Merge or delete `[mcp_servers.<serverName>]` in a codex config.toml. Returns whether it changed. */
export function applyCodexEntry(file: string, serverName: string, command: string | null, args: string[]): boolean {
    const fileExists = existsSync(file);
    ...
    const head = removeTomlTable(before, `mcp_servers.${serverName}`).replace(/\s*$/, "");
    let after: string;
    if (command !== null) {
...
/**
 * Write/remove one managed MCP server entry (`serverName`, launched by `inv`) in `file`
 * for `tool`, per `enabled`. Shared by the enigma server and any other managed server.
 */
export function writeServerEntry(tool: string, file: string, serverName: string, inv: { command: string; args: string[] }, enabled: boolean): boolean {
    switch (tool) {
        case "claude":
            return applyJsonEntry(file, "mcpServers", serverName, enabled ? { type: "stdio", command: inv.command, args: inv.args } : null);
        case "opencode":
            return applyJsonEntry(file, "mcp", serverName, enabled ? { type: "local", command: [inv.command, ...inv.args], enabled: true } : null);
        case "codex":
            return applyCodexEntry(file, serverName, enabled ? inv.command : null, inv.args);
        default:
            return false;
    }
}
...
/** The agents whose own config can host a managed MCP server. */
export const MANAGED_TOOLS = ["claude", "codex", "opencode"] as const;
