...
const FIX_POLICY_HELP: Record<FixPolicy, string> = {
    ...
    assisted: "fix_policy is `assisted`: findings the review could settle mechanically are already handled, so this gate carries a judgment call. Escalate an ask-user finding only when the answer is not already yes - fixing it would contradict what the user asked for, undo a deliberate decision, change agreed behavior, or take a very large change. Otherwise authorize the fix yourself: `should I fix this?` is not a question to hand back.",
