...
function gateMessage(reason: VerifyGap): string {
    return [
        ...
        "Authorize `auto-fix` and `no-op` findings on your own judgment. An `ask-user` finding goes back to the user only when fixing it would contradict what they asked for, undo a deliberate decision, change agreed behavior, or take a very large change; otherwise authorize that fix yourself too, because \"should I fix this?\" is not a question to hand back. Never pass `--yes`. `checks-passed` means the run is now waiting on the USER to merge, not on you and not on the pipeline: report it that way rather than as still running, and merge it yourself (`enigma gate axi merge`) only if the user asked you to.",
