...
/**
 * A right-click menu built by hand: the handler, the pointer coordinates, and the state that
 * holds them.
 *
 * All THREE, because each on its own is something else entirely. `onContextMenu` alone is
 * usually a canvas or an editor suppressing the browser's menu; `clientX` alone is a drag, a
 * tooltip or a marquee; a `setMenu` alone is a dropdown with no pointer behind it. Together
 * they are only ever one thing, and that thing has a long tail nobody writes on the first
 * pass - the release of the opening press choosing whatever row it landed on, the submenu
 * that closes while the pointer crosses a sibling to reach it, the panel that opens off the
 * bottom of the window, Escape, Shift+F10, the scroll that leaves the menu behind.
 */
const MENU_HANDLER = /onContextMenu|addEventListener\(\s*["'`]contextmenu["'`]|@contextmenu|on:contextmenu/;
const MENU_POINT = /\bclient[XY]\b|\bpage[XY]\b/;
/** The state a menu opened at the pointer has to hold. A setter, not a noun: names are cheap. */
const MENU_STATE = /\bset(?:ContextMenu|Menu(?:Pos|Position|Point|Anchor|Coords|Origin|At)?)\b|\bmenu(?:Pos|Position|Point|Anchor|Coords)\s*[:=]/i;
/** Already using a menu component - this package's, or anyone else's. */
const MENU_MITIGATED = /@enigmax\/primitives|createContextMenu|useContextMenuContext|<ContextMenu|ContextMenu\.|radix-ui|@headlessui|@szhsin\/react-menu|primereact|@mui\/material|antd|enigma:allow-hand-rolled-menu/;

export function handRolledContextMenu(content: string): { line: number; detail: string; }[] {
    ...
    for (let i = 0; i < lines.length; i++) {
        ...
        // One finding per file: there is one menu in it, and one fix.
...
/**
 * A file-manager selection built by hand: Shift for a range, Ctrl to add, and a set of ids.
 *
 * The same three-part rule as the menu above. `shiftKey` alone is a shortcut; a `Set` of ids
 * alone is a filter; Ctrl alone is a shortcut too. Together they are Explorer's selection
 * model, and what goes wrong with it is always the same short list: the Shift+click measured
 * from the last CLICK instead of the anchor (so dragging the shift-click leaves a trail of
 * rows behind), the Ctrl+click that clears the selection, the range that walks over a row it
 * was not allowed to select, the arrow keys that never arrive, and Escape doing nothing.
 */
const SELECT_RANGE_KEY = /\bshiftKey\b/;
const SELECT_ADD_KEY = /\bctrlKey\b|\bmetaKey\b/;
/** The selection itself: a set or list of ids the rows are checked against. */
const SELECT_STATE = /\bset(?:Selected|Selection|SelectedIds|SelectedRows|Checked)\b|\bselected(?:Ids|Rows|Keys|Items)\b/;
const SELECT_MITIGATED = /@enigmax\/primitives|createSelection|useSelection|<SelectionList|enigma:allow-hand-rolled-selection/;

export function handRolledSelection(content: string): { line: number; detail: string; }[] {
...
/**
 * A colour picker built by hand: colour-space maths, a drag measured off a box, and the
 * colour it is steering.
 *
 * Three parts again, and they separate the same way. The conversions alone are a theme file
 * or a chart palette. A pointer drag measured off `getBoundingClientRect` alone is a slider,
 * a splitter or a canvas. A `setColor` alone is a form field. Together they are a saturation
 * square, and what goes wrong with one is always the same thing: hue is undefined at black,
 * at white and at every grey, so a picker that re-derives HSV from its own RGB resets the
 * rail to red the moment the square is dragged into a corner.
 */
const COLOR_MATHS = /\b(?:hsv|hsl|hwb|rgb|hex)(?:To|2)(?:Rgb|Hsv|Hsl|Hwb|Hex|String)\b/i;
/** A press whose position is read off the control's own box - the drag, not a click. */
const COLOR_DRAG = /onPointerDown|onMouseDown|@pointerdown|@mousedown|on:pointerdown|on:mousedown|addEventListener\(\s*["'`](?:pointerdown|mousedown)["'`]/;
const COLOR_MEASURE = /getBoundingClientRect/;
/** The colour under the pointer: a setter, or the two components a square and a rail move. */
const COLOR_STATE = /\bset(?:Color|Colour|Hue|Saturation|Lightness|Brightness|Alpha|Opacity)\b|\b(?:hue|saturation)\s*[:=]/i;
const COLOR_MITIGATED = /@enigmax\/primitives|type=["']color["']|react-colorful|react-color|@uiw\/react-color|vue-color|ngx-color|@radix-ui|enigma:allow-hand-rolled-color-picker/;

export function handRolledColorPicker(content: string): { line: number; detail: string; }[] {
...
/**
 * A lightbox built by hand: a picture, a wheel that scales it, the zoom kept as state, and the
 * dialog all of it sits in.
 *
 * FOUR parts, because each of the first three is ordinary on its own. An `<img>` is an image. A
 * wheel handler is a chart, a map or a scroller. A scale in state is any transform. What makes it
 * a viewer is all three plus the dialog - an overlay, an Escape, or a portal.
 */
const LIGHTBOX_IMAGE = /<img\b|<Image\b|createElement\(\s*["'`]img["'`]/;
const LIGHTBOX_WHEEL = /onWheel|@wheel|on:wheel|addEventListener\(\s*["'`]wheel["'`]/;
/** The zoom itself: a scale in state, or one written into a transform. */
const LIGHTBOX_ZOOM = /\bset(?:Scale|Zoom)\b|\b(?:scale|zoom)\s*[:=]\s*[0-9(]|scale\(/;
/** The dialog around it, in any of the spellings a lightbox uses. */
const LIGHTBOX_DIALOG = /role=["']dialog["']|aria-modal|lightbox|createPortal|["']Escape["']|backdrop/i;
const LIGHTBOX_MITIGATED = /@enigmax\/primitives|data-enigma-image|ImageViewer\b|yet-another-react-lightbox|react-image-lightbox|photoswipe|medium-zoom|viewerjs|fancybox|lightgallery|react-medium-image-zoom|enigma:allow-hand-rolled-lightbox/i;

export function handRolledLightbox(content: string): { line: number; detail: string; }[] {
...
/**
 * A video player built by hand: the element, its playback driven by code, and a bar of controls
 * drawn over it.
 *
 * `<video controls>` is the browser's own player and is left alone - this rule is about REPLACING
 * that bar. So the signature needs the element, a call that plays or pauses it, the timeline it
 * reads, and a control of its own to show for it.
 */
const VIDEO_ELEMENT = /<video\b|createElement\(\s*["'`]video["'`]|HTMLVideoElement/;
const VIDEO_DRIVEN = /\.play\(\)|\.pause\(\)|\bplaybackRate\b|requestPictureInPicture/;
/**
 * The timeline the bar is drawn from. `timeupdate` is matched as a bare word rather than as a
 * quoted string: it is a media event name and nothing else, and every framework spells the
 * binding its own way (`onTimeUpdate`, `@timeupdate`, `on:timeupdate`).
 */
const VIDEO_TIMELINE = /\bcurrentTime\b|timeupdate|\.buffered\b/i;
/** A bar of its own: a play button, a scrubber, or a fullscreen control the page draws. */
const VIDEO_CONTROLS = /aria-label=["'](?:Play|Pause|Mute|Seek|Fullscreen)|onClick=\{[^}]*(?:play|pause)|requestFullscreen|toggleFullscreen|isPlaying|setPlaying/i;
/**
 * Case-INSENSITIVE, so the component's own marker cannot be listed here: `<Video` would match
 * `<video` and exempt every player there is. `data-enigma-video` is the marker instead, which is
 * what the primitive renders and nothing else does.
 */
const VIDEO_MITIGATED = /@enigmax\/primitives|data-enigma-video|["'`]plyr|from\s+["'`]plyr|video\.js|videojs|react-player|vidstack|media-chrome|@vime|shaka-player|enigma:allow-hand-rolled-video-player/i;

export function handRolledVideoPlayer(content: string): { line: number; detail: string; }[] {
