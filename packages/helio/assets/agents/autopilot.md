...
- Q1: Can attacker do this RIGHT NOW? Must have exact request/response.
  OSS EXCEPTION: public codebase + passive version confirmation + payload labeled "NOT EXECUTED"
- Q2-Q7: Standard validation gates (see agents/validator.md)

A finding is only "Confirmed" when ALL hold:
- Exact HTTP request that reproduces it
- Actual response showing impact (not just 200 OK or DNS callback)
- Reproduced from fresh session at least twice
- Reachability classified (EXTERNAL / AUTHENTICATED / INTERNAL)
- Exploitability classified (EASY / MEDIUM / HARD)
- Confidence >= 70
