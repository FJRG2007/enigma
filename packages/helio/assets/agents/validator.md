...
- OSS EXCEPTION: "OSS target — code path traced + vulnerable version confirmed deployed passively (header/version endpoint/package metadata). Payload exists but not executed in production." → PASS Q1 with OSS label
- NO: "Researcher only read code, no confirmed PoC, no version confirmation" → KILL Q1
...
- "To trigger this, attacker must first exploit [other bug]" → KILL Q1 — dependency on another vulnerability makes this not standalone. Confirm and report the full chain as one finding only if BOTH bugs are confirmed.

## OSS Scope Exception

When the program includes a public open-source codebase AND a production domain, Q1 can be satisfied by **two-path evidence** instead of a full triggered PoC:

1. **Code evidence:** Full vulnerable code path traced in the public repo.
2. **Production evidence (passive only):** Deployed version confirmed via one of:
   - Response headers (`X-Runtime`, `Server`, `X-Powered-By`, `X-Version`)
   - Public version endpoint (`/version`, `/health`, `/api/version`)
   - npm / PyPI / Docker Hub / package metadata
   - Publicly served `package.json` or `requirements.txt`

When both paths confirmed → mark finding as `OSS-CONFIRMED`. Include payload labeled "NOT EXECUTED IN PRODUCTION."

Hard limits even under this exception:
- NEVER execute the exploit payload in production
- NEVER trigger data exfil, state change, or disruption
- NEVER spray production to confirm — passive version check only

If version cannot be confirmed deployed → Q1 fails. Kill it.
...
When killing a finding, name which platform rejection reason it maps to so the researcher understands:

| Kill Reason | Platform Rejection Label |
|---|---|
| Cannot show HTTP request / no PoC | NOT REPRODUCIBLE |
| CVSS < 4.0 / on never-submit list | INFORMATIONAL |
| Asset not in scope / third-party service | OUT OF SCOPE |
| Already disclosed / in changelog | SPAM / DUPLICATE |
| Framework prevents it / dead code / documented behavior | INVALID |
...
REJECTION CATEGORY: [NOT REPRODUCIBLE / INFORMATIONAL / OUT OF SCOPE / SPAM / INVALID]
