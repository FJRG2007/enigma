...
**OSS Scope Exception:** If the program has a public codebase AND a production domain, Q6 is satisfied by two-path evidence (no exploit trigger required):
1. Full vulnerable code path traced in the public repo
2. Deployed version confirmed via passive check only: response headers, `/version`/`/health` endpoint, npm/PyPI/Docker Hub metadata, or publicly served `package.json`/`requirements.txt`
3. Payload included in report labeled "NOT EXECUTED IN PRODUCTION — code-confirmed"

Hard limits even under this exception: NEVER send the actual exploit to production, trigger data exfil, or modify state. Passive version confirmation only. If deployed version cannot be confirmed → Q6 fails.
...
[ ] Bug is REAL — confirmed with actual HTTP requests, not code reading alone
    OSS EXCEPTION: code path traced in public repo + vulnerable version confirmed deployed (passive only)
[ ] Bug is IN SCOPE — checked program scope page explicitly
[ ] Reproducible from scratch — can reproduce starting from fresh session
    OSS EXCEPTION: code path is reproducible (traced) + deployed version confirmed
[ ] Evidence ready — screenshot, response body, or video
    OSS EXCEPTION: code excerpt + version confirmation evidence (header/endpoint/package metadata)
```
...
7. **Vuln dependency**: "To reach this bug, attacker must first exploit [other bug]" → kill it as standalone. It is only reportable as a confirmed end-to-end chain where BOTH vulnerabilities are independently verified. If you can't confirm both → report only the prerequisite bug.
