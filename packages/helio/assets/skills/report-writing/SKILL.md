...
## CHARACTER RULES (ASCII only — applies to all TUIs, all report types, always)

These rules apply whether the report is triggered by /report, /autopilot, or a plain conversation request. No exceptions.

In prose AND in code comments:
- Use -> not → (unicode right arrow)
- Use => not ⇒
- Use <- not ←
- Use -- not — (em-dash)
- Use ... not … (ellipsis)
- Use ---- not ──── or ═══ or any box-drawing character
- No emojis: not in prose, not in titles, not in code comments, not anywhere
- No unicode characters a developer would not type from a standard keyboard
...
- Summary and impact are prose paragraphs, not bullet lists
- One header level max for single-bug reports
- Active voice, short sentences

## POC REQUIREMENT

Every report includes a working PoC. For automated flows (autopilot, hunt), derive from captured request/response data. Never skip it. No author comment inside the PoC code.

## HUMAN TONE GUIDELINES

**Write to a person, not a chatbot:**
