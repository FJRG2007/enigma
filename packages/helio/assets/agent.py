...
class HuntMemory:
    ...
    def add_observation(self, tool: str, text: str) -> None:
        ...
        if len(self.observation_buf) > 15: self.observation_buf = self.observation_buf[-10:]
    ...
    def findings_summary(self) -> str:
        ...
        if not self.findings_log: return "No findings yet."
    ...
    def recent_observations(self, n: int = 3) -> str:
        ...
        if not recents: return "No tool outputs yet."
        parts = []
        for obs in recents:
...
class ToolDispatcher:
    ...
    def __init__(self, domain: str, memory: HuntMemory, scope_lock: bool = False, max_urls: int = 100, default_cookies: str = ""):
        self.domain          = domain
        self.memory          = memory
    ...
    def dispatch(self, name: str, args: dict) -> str:
        ...
        try:
            if name == "run_recon":
                ...
                if not req_file or not os.path.isfile(req_file): return f"ERROR: request_file not found: {req_file}"
            ...
            elif name == "read_recon_summary": obs = self._read_recon_files(domain)

            elif name == "read_findings_summary": obs = self._read_findings_files(domain)
            ...
            elif name == "finish": return f"FINISH: {args.get('verdict', 'Hunt complete.')}"
            ...
            for root, _, files in os.walk(findings_dir):
                for fn in files:
                    ...
                    try:
                        ...
                        if any(kw in content.lower() for kw in ("critical", "high", "vulnerable", "injectable", "rce", "sqli", "open redirect", "exposed", "default cred")):
                            head = content[:400].replace("\n", " ")
                            lines.append(f"  [{fn}] {head}")
                    except Exception: pass

        if len(lines) == 1: lines.append("  No HIGH/CRITICAL findings in artifacts (check logs above for details).")
        return "\n".join(lines[:20])
    ...
    def _read_findings_files(self, domain: str) -> str:
        h = _h()
        findings_dir = h._resolve_findings_dir(domain, create=False)
        if not findings_dir or not os.path.isdir(findings_dir): return "No findings directory. Run vulnerability scans first."

        parts = []
        for root, _, files in os.walk(findings_dir):
            for fn in sorted(files):
                if not fn.endswith((".txt", ".json")): continue
                fp = os.path.join(root, fn)
                try:
                    content = Path(fp).read_text(errors="replace")
                    if content.strip():
        ...
        if not parts: return "Findings directory exists but is empty."
        combined = "\n\n".join(parts)
        # Truncate to avoid blowing context
        if len(combined) > MAX_CTX_CHARS: combined = combined[:MAX_CTX_CHARS] + "\n...[truncated]"
        return combined

    def _classify_obs(self, tool: str, obs: str) -> None:
        """Extract severity labels from observation text and add to findings_log."""
        obs_l = obs.lower()
        if any(kw in obs_l for kw in ("rce_confirmed", "injectable", "critical")): sev = "CRITICAL"
        elif any(kw in obs_l for kw in ("high", "sql injection", "rce", "default cred")): sev = "HIGH"
        elif any(kw in obs_l for kw in ("medium", "exposed", "open redirect", "cors")): sev = "MEDIUM"
        elif any(kw in obs_l for kw in ("low", "info")): sev = "LOW"
        else: return  # not a finding, skip

        # Take first relevant line as summary
        for ln in obs.splitlines():
            if any(kw in ln.lower() for kw in ("critical", "high", "injectable", "rce", "exposed", "found", "medium", "sql")):
...
class AgentTracer:
    ...
    def finish(self, verdict: str, step: int, elapsed_mins: float) -> None:
        self._write({"event": "finish", "step": step, "elapsed_mins": elapsed_mins, "verdict": verdict})
...
def race_analysis(prompt: str, models: list[str], client, system: str = "", timeout: int = 120) -> str:
    """
...
class ReActAgent:
    ...
    def __init__(self, domain: str, memory: HuntMemory,
        ...
        if not _OLLAMA_OK: raise RuntimeError("Ollama Python package not installed: pip install ollama")

        self.client = _ollama_lib.Client(host=OLLAMA_HOST)
        self.model  = model or self._pick_tool_capable_model()
        if not self.model: raise RuntimeError("No Ollama model available. Pull one: ollama pull qwen2.5:32b")

        # Build race roster: primary model + baron-llm if available and different
        try:
            available = [m.model for m in self.client.list().models]
            if "baron-llm:latest" in available and "baron-llm:latest" != self.model: self._race_models = [self.model, "baron-llm:latest"]
            else: self._race_models = [self.model]
        except Exception: self._race_models = [self.model]
        ...
        print(f"{DIM}[Agent] max_steps={max_steps}  budget={time_budget_hours}h  " f"tool_calling=native{race_note}{NC}", flush=True)
    ...
    def _pick_tool_capable_model(self) -> str | None:
        ...
        try: available = [m.model for m in self.client.list().models]
    ...
    def _check_bump(self) -> str | None:
        ...
        if not self.bump_file or not os.path.isfile(self.bump_file): return None
        try:
    ...
    def step(self) -> str | None:
        ...
        if bump_msg:
            ...
            if self.tracer: self.tracer.bump(bump_msg, self.memory.step_count)
        ...
        if tool_calls:
            ...
            for tc in tool_calls:
                ...
                if name == "finish" and self.memory.step_count < self.MIN_STEPS_BEFORE_FINISH:
                    ...
                    print(f"{YELLOW}[Agent] Finish blocked — only {self.memory.step_count} steps done, " f"need {remaining_needed} more. Continuing...{NC}", flush=True)
                    results.append(
                ...
                if must_break:
                    print(f"{RED}[Agent] Loop detected on '{name}' — forcing direction change{NC}", flush=True)
                    if self.tracer: self.tracer.loop_break(name, self.memory.step_count)
                    ...
                    continue
                if warn:
                    print(f"{YELLOW}[Agent] Loop warning: '{name}' repeated — consider switching{NC}", flush=True)
                    if self.tracer: self.tracer.loop_warn(name, LoopDetector.WARN_AT, self.memory.step_count)

                print(f"{MAGENTA}[Agent] Tool: {BOLD}{name}{NC}{MAGENTA}  args={json.dumps(args)}{NC}",
                      flush=True)
                if self.tracer: self.tracer.tool_call(name, args, self.memory.step_count)

                t0  = time.time()
                obs = self.dispatcher.dispatch(name, args)
                elapsed = round(time.time() - t0, 1)

                if self.tracer: self.tracer.tool_result(name, obs, elapsed, self.memory.step_count)
                ...
                if name == "finish":
                    ...
                    if self.tracer:
                        self.tracer.finish(self.verdict, self.memory.step_count, round((time.time() - self.time_start) / 60, 1))
    ...
    def run(self) -> dict:
        ...
        if self.tracer: print(f"  Trace log:       {self.tracer.log_path}")
        if self.bump_file: print(f"  Bump file:       {self.bump_file}")
        if self.verdict: print(f"  Verdict:         {self.verdict}")
        ...
        return {
            ...
            **{step: (step in self.memory.completed_steps)
               for step in ("recon", "scan", "js_analysis", "secret_hunt", "param_discovery", "api_fuzz", "cors", "cms_exploit", "rce_scan", "sqlmap", "jwt_audit")},
        }
...
def build_langgraph_agent(domain: str, dispatcher: ToolDispatcher,
    ...
    def agent_node(state: HuntState) -> HuntState:
        ...
        msgs = state["messages"]
        if not any(isinstance(m, SystemMessage) for m in msgs): msgs = [SystemMessage(content=AGENT_SYSTEM), HumanMessage(content=context)] + list(msgs)
        response = llm_with_tools.invoke(msgs)
        # Check finish signal
        if hasattr(response, "tool_calls"):
            for tc in (response.tool_calls or []):
                if tc.get("name") == "finish": memory.working_memory += f"\n\nFINISHED: {tc.get('args', {}).get('verdict', '')}"
    ...
    def should_continue(state: HuntState):
        ...
        if not hasattr(last, "tool_calls") or not last.tool_calls: return END
        if any(tc.get("name") == "finish" for tc in last.tool_calls): return END
        if memory.step_count >= max_steps: return END
        return "tools"
