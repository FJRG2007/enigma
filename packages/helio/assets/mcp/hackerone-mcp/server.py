...
def search_disclosed_reports(
    ...
    if keyword:
        ...
        where_clauses.append(f'report: {{ title: {{ _icontains: "{safe_keyword}" }} }}')
    if program:
        safe_program = program.replace('"', '\\"')
        where_clauses.append(f'team: {{ handle: {{ _eq: "{safe_program}" }} }}')
...
def get_program_stats(program: str) -> dict:
    ...
    if not team: return {"error": f"Program '{program}' not found", "program": program}
