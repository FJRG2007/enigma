...
def gql(cookie: str, csrf: str, query: str, variables: dict = None) -> tuple[int, dict]:
    ...
    try:
        ...
        except Exception: return e.code, {"_raw": e.read().decode(errors="replace")}
...
def check(label: str, status: int, resp: dict) -> bool:
    ...
    finding = False
    if has_data and not errors: finding = True
    if has_data and errors and not any("not authorized" in m.lower() or "permission" in m.lower() or "not found" in m.lower() for m in err_msgs): finding = True

    marker = "[!!FINDING!!]" if finding else "[BLOCKED]   "
    print(f"\n  {marker} {label}")
    print(f"  HTTP {status} | errors={err_msgs[:2] if err_msgs else 'none'}")
    if has_data: print(f"  data={json.dumps(data)[:200]}")
    return finding
...
def main():
    ...
    if check(f"Read report {rid} as B", status, resp): findings.append("Read report")
    time.sleep(0.5)
    ...
    status, resp = gql(args.cookie_b, csrf_b, q)
    if check("updateReportTitle as B", status, resp): findings.append("updateReportTitle")
    time.sleep(0.5)
    ...
    status, resp = gql(args.cookie_b, csrf_b, q)
    if check("updateReportVulnerabilityInformation as B", status, resp): findings.append("updateReportVulnerabilityInformation")
    time.sleep(0.5)
    ...
    status, resp = gql(args.cookie_b, csrf_b, q)
    if check("updateReportSeverity as B", status, resp): findings.append("updateReportSeverity")
    time.sleep(0.5)
    ...
    status, resp = gql(args.cookie_b, csrf_b, q)
    if check("updateReportStateToTriaged as B", status, resp): findings.append("updateReportStateToTriaged")
    time.sleep(0.5)
    ...
    status, resp = gql(args.cookie_b, csrf_b, q)
    if check("closeReport as B", status, resp): findings.append("closeReport")
    time.sleep(0.5)
    ...
    status, resp = gql(args.cookie_b, csrf_b, q)
    if check("requestPublicDisclosure as B", status, resp): findings.append("requestPublicDisclosure")
    time.sleep(0.5)
    ...
    for att in attachments[:3]:
        ...
        if att_id:
            ...
            if check(f"Attachment {att_id} as B", status2, resp2): findings.append(f"attachment-{att_id}")
        time.sleep(0.3)
    ...
    status, resp = gql(args.cookie_b, csrf_b, q)
    if check("user(id) public profile as B", status, resp): findings.append("user public profile")
    time.sleep(0.5)
    ...
    if check("User earnings via node(GID) as B", status, resp): findings.append("user earnings via GID")
    time.sleep(0.5)
    ...
    status, resp = gql(args.cookie_b, csrf_b, q)
    if check("awardBounty as B on A's report", status, resp): findings.append("awardBounty")
    time.sleep(0.5)
    ...
    status, resp = gql(args.cookie_b, csrf_b, q)
    if check("shareReportViaEmail as B", status, resp): findings.append("shareReportViaEmail")
    time.sleep(0.5)
