...
- CHARTS: the per-day "tokens saved" graph is TradingView Lightweight Charts (`AreaSeries`, v5 API `chart.addSeries(LC.AreaSeries, ...)`), VENDORED as a static asset at `assets/dashboard/lib/lightweight-charts.standalone.production.js` (Apache-2.0, ~196 KB) and served by the loopback server at `/lib/<file>` (`dashboardLib()` cached read; 404 when absent and the cards still render). DELIBERATELY not a CDN at runtime and NOT an npm runtime dependency - the zero-dependency posture holds because it is a local static file shipped in the tarball (`assets` already in package.json `files`) and read env-first via `ENIGMA_ASSETS_DIR` like the HTML. The HTML buckets history into ascending unique-per-day `{time:'YYYY-MM-DD', value}` (UTC day via `toISOString().slice(0,10)`, matching the server's UTC-day points), a 7D/30D/90D/All range switcher drives `timeScale().setVisibleRange`/`fitContent`, and a crosshair tooltip shows the hovered date + saved tokens. WHY vendor vs CDN: anti-CDN/ciphera-style policy forbids a runtime CDN; vendoring is the only policy-compliant way to honor an explicit "use lightweight-charts" request. To bump the lib: re-download the same `*.standalone.production.js` from unpkg at the new version (the only fetch is at vendor time). The minified blob passes `npm run guard` (no secret false-positives). UI text is Title Case throughout (wordmark "Enigma").
- BY-SOURCE PANEL: "Savings By Source" renders `stats.bySource` (see the compression-engine note's SOURCE ATTRIBUTION bullet) as one bar per origin sorted by tokens saved, with `prettySource` friendly labels and a calls count. Empty -> a hint that attribution starts on the next compression. The `/api/stats` payload already carries `bySource` (it serializes `readStats()` verbatim), so no server change was needed - only the HTML `render()` calling `renderSources(s.bySource)`.
