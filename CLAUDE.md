...
- Run `npm run seal`, then `node --check` and the `--dry-run` preview. Current count: 18 skills (16 policy skills + the vendored Anthropic `frontend-design` and `skill-creator` skills; the vendored ones carry only `name`/`description`/`version` and are re-provider'd to FJRG2007/enigma by seal, and their LICENSE.txt is intentionally not bundled).
