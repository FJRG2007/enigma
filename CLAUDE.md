...
- TOOL-AGNOSTIC by design but ONLY Claude Code is wired up. The `TOOLS` registry in `accounts.ts` holds one `ToolSpec` (claude: `CLAUDE_CONFIG_DIR`, bin `claude`, `defaultDir ~/.claude`). Adding Codex/opencode later = one registry entry (`envFor` returns a `Record`, so opencode's `XDG_DATA_HOME`+`XDG_CONFIG_HOME` pair fits; codex = `CODEX_HOME`, `loginArgs:["login"]`). `enigma <tool>` and `--tool` light up automatically from `TOOL_NAMES`. Do NOT hardcode "claude" in new code - go through `getTool`/`isToolName`.
